...
if($_SERVER['REQUEST_METHOD'] == 'POST' & isset($_POST['login']))
{
	...
	else
	{
		...
		if($conn2)
		{ 
			$query = mysql_query("SELECT * FROM failedAttempts WHERE ip = '$userIP' AND lastAttempt > '$date' AND type = '1'");
			$failCount = 0;
			if($query)
			{
			...
			else
			{
				...
				if($count > 0)
				{
					//mysql_query("DELETE FROM failedattempts WHERE ip = '$userIP' AND type ='1'");
					$row = mysql_fetch_array($query1);
					$lastLogin = $row["loginTime"];
					$lastIP = $row["IP"];
					$md5Pass = $row["MD5Password"];
					mysql_query("UPDATE accounts SET lastLogin = '$lastLogin', lastLoginIP = '$lastIP' WHERE LoginID = '$username'") or die(mysql_error());
					mysql_query("UPDATE accounts SET IP = '$userIP', loginTime = '$date' WHERE LoginID = '$username'") or die(mysql_error());
					
					if(isset($_COOKIE['loginCookie']) && $row["loginCookie"] != "")
					{
						if($row["loginCookie"] == $_COOKIE['loginCookie'])
						{
							$data = "INSERT INTO LoginLogs (LoginID, Time, IP, Verify) VALUES ('$username', '$date', '$userIP', '1')";
							mysql_query("$data");
						}
						else
						{
							$data = "INSERT INTO LoginLogs (LoginID, Time, IP, Verify) VALUES ('$username', '$date', '$userIP', '2')";
							mysql_query("$data");
						}	
					}
					else
					...
					if(isset($remember))
					{
						...
						setcookie("Password", $md5Pass, time()+3600*24*365*10,  '/');
					}
					else
					{
						setcookie("LoginID", $username, time()+3600,  '/'); 
						setcookie("Password", $md5Pass, time()+3600,  '/'); 
					}
					die("Login");
				...
				{
					mysql_query("INSERT INTO failedattempts (username, ip, lastAttempt,type) VALUES ('$username', '$userIP', '$timeout', '1')");
