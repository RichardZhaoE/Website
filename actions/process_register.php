				...
				if(filter_var($email, FILTER_VALIDATE_EMAIL))
  				{
					...
					else
					{
						...
						//$query1 = mysql_query("SELECT * FROM accounts WHERE IP = '$userIP' OR lastLoginIP = '$userIP'");
						//$count = mysql_num_rows($query1);
						//if($count == 0)
						//{
							$result = mysql_query("SELECT COUNT(*) AS Total,
								SUM(IF(LOWER(LoginID) = LOWER('$username'), 1, 0)) AS userIDCount,
								SUM(IF(LOWER(Email) = LOWER('$email'), 1, 0)) AS emailNum FROM accounts");
							if($result === FALSE)
							{
								die(mysql_error());
							}
							$query = mysql_fetch_array($result);
							$num1 = $query[userIDCount];
							$num2 = $query[emailNum];
							if($num1 == 0)
							{
								if($num2 == 0)
								{
									...
									$reg = "INSERT INTO accounts (LoginID, Password, MD5Password, Expires, IP, lastLoginIP, lastLogin, Email, Admin, loginTime, dateCreated, approveDate, World) 
									VALUES ('$username', '$password', '$md5Pass', '$date', '$userIP', '$userIP', '$date', '$email','0', '$date',  '$date',  '$date', '$world')";
									mysql_query($reg);
									die("Register");
								}
								else
